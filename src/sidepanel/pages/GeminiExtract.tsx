// ─── Gemini Extraction Page ─────────────────────────────────
// Extracts Gems from the user's Gemini account via the
// batchexecute API (GEMINI_EXTRACT_GEMS message to content script).
...
async function findGeminiTab(): Promise<number> {
  ...
  if (tabId === undefined) {
    throw new Error(
      "No Gemini tab found. Please open gemini.google.com and sign in.",
    );
  }
  return tabId;
}
...
export default function GeminiExtract(): React.JSX.Element {
  ...
  const [steps, setSteps] = useState<TrackedStep[]>([
    { id: "gems", label: "Fetching your Gems...", status: "pending" },
  ...
  const checkForGemini = useCallback(async () => {
  ...
  const runExtraction = useCallback(async () => {
    ...
    try {
      ...
      if (!result.success && result.gems.length === 0) {
        const warning = result.warnings[0]?.message ?? "Extraction failed";
        throw new Error(warning);
      }

      const gemCount = result.gems.length;
      markStep(
        0,
        "complete",
        `Found ${gemCount} Gem${gemCount === 1 ? "" : "s"}`,
      );
    ...
    return (
      <div className="flex flex-1 flex-col">
        <h2 className="text-lg font-semibold text-gray-900">
          Reading from your Gemini account
        </h2>
        <p className="mt-1 text-sm text-gray-500">
          We'll fetch your custom Gems from Gemini via its internal API.
        </p>
              ...
              <div className="space-y-2">
                <button
                  onClick={async () => {
                    ...
                    setTimeout(() => void checkForGemini(), 3000);
                  }}
          ...
          {tabStatus === "not_responding" && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
              <p className="text-sm font-medium text-amber-800 mb-2">
                Found Gemini, but can't connect to it
              </p>
              <p className="mb-3 text-xs text-amber-600">
                Try refreshing your Gemini tab, then click the button below.
              </p>
              <button
                onClick={() => void checkForGemini()}
                className="w-full rounded-md bg-amber-100 px-3 py-2 text-sm font-medium text-amber-800 transition-colors hover:bg-amber-200"
        ...
        >
          {tabStatus === "ready" ? "Start Reading" : "Waiting for Gemini..."}
        </button>
      </div>
    );
  }
  ...
  if (phase === "error") {
    ...
    if (error?.includes("No Gemini tab found")) {
      friendlyError =
        "We couldn't find Gemini open in any of your tabs. Please open gemini.google.com, sign in, then try again.";
    } else if (error?.includes("SNlM0e")) {
      friendlyError =
        "Couldn't authenticate with Gemini. Please make sure you're signed in to gemini.google.com, then try again.";
    }
